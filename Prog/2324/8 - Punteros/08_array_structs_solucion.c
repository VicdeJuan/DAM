...
char * toString_warning(Estudiante estu){
...
char * toString(Estudiante estu,char * retval){
	// Mejor que el 100 sería un define.
	snprintf(retval,MAX_BUFFER*sizeof(char),"Estudiante %s tiene %d años y ha sacado un %f",estu.nombre,estu.edad,estu.nota);

	//snprintf(retval,sizeof(retval),"Estudiante %s tiene %d años y ha sacado un %f",estu.nombre,estu.edad,estu.nota);
}
...
int main() {
	...
	for (int i = 0; i<num; i++){
   		printf("%s\n\n",toString_warning(listado[i])); 
		toString(listado[i],buffer);
		printf("%s\n\n",buffer);
	}
